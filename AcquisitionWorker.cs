...
namespace simple_live_wpf
{
    public class AcquisitionWorker
    {
        ...
        // Event which is raised if the counters have changed
        ...
        public void Start()
        {
            ...
            while (running)
            {
                try
                {
                    ...
                    buffer = dataStream.WaitForFinishedBuffer(10000000000);
                ...
                {
                    if (e.Message.Contains("PEAK_RETURN_CODE_ABORTED"))
                    {
                        Debug.WriteLine("--- [AcquisitionWorker] acq. aborted: " + e.Message);
                        running = false;
                        break;
                    }
                    else
